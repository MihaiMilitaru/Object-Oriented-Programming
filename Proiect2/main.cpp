...
public:
    ...
    void calc_rating() override {
        Malware::calc_rating();
        rating += rating_ob + float(rating_cript);
    }
};
...
public:
    ...
    const vector<shared_ptr<Malware>> &getVirusi() const;
...
istream &Computer::read(istream &is) {
    id_pc=id++;
    cout<<"Introdu m virusi.\n";
    cout<<"m= ";
    int m;
    cin>>m;
    if(m!=0){
        for(int i=0;i<m;i++){
            float rating_total_curent=0;
            cout<<"Introduceti:\n 1. Rootkit\n 2. Keylogger\n 3. KernelKeylogger\n 4. Ransomware\n";
            int tip;
            cin>>tip;
            while(tip!=1 and tip!=2 and tip!=3 and tip!=4){
                cout<<"Tip invalid. Reintrodu tipul de virus.\n";
                cin>>tip;
            }
            if(tip==1){
                Rootkit virus;
                cin>>virus;
                virus.calc_rating();
                virusi.push_back(make_shared<Rootkit>(virus));
                rating_total_curent+=virus.getRating();
            }else if(tip==2){
                Keylogger virus;
                cin>>virus;
                virus.calc_rating();
                virusi.push_back(make_shared<Keylogger>(virus));
                rating_total_curent+=virus.getRating();
            }else if(tip==3){
                KernelKeylogger virus;
                cin>>virus;
                virus.calc_rating();
                virusi.push_back(make_shared<KernelKeylogger>(virus));
                rating_total_curent+=virus.getRating();
            }else if(tip==4){
                Ransomware virus;
                cin>>virus;
                virus.calc_rating();
                virusi.push_back(make_shared<Ransomware>(virus));
                rating_total_curent+=virus.getRating();
            }
...
const vector<shared_ptr<Malware>> &Computer::getVirusi() const {
    return virusi;
}
...
public:
    ...
    void option2(){
        for(int i=0;i<calc.size()-1;i++){
            for(int j=i+1;j<calc.size();j++){
                ...
                float rating2=calc[j].getRatingTot();
                if(rating1>rating2){
                    swap(calc[i], calc[j]);
                }
            }
    ...
    void option4() {
        ...
        int nr_calc=calc.size();
        int nr_calc_infectate=0;
        for(int i=0;i<calc.size();i++){
            if(calc[i].getVirusi().size()){
                nr_calc_infectate++;
            }
        }
        float procent=float(nr_calc_infectate)/nr_calc*100;
        cout<<procent<<" %\n";
    }
